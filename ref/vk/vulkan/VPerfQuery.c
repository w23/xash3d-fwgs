...
int vPerfQueryBegin(VPerfQuery *pq, struct vk_combuf_s *cb) {
	for (uint32_t i = 0; i < pq->queries.max; ++i) {
		...
		vkCmdResetQueryPool(cb->cmdbuf, pq->pool, i, 1);
		vkCmdBeginQuery(cb->cmdbuf, pq->pool, i, 1);
...
void vPerfQueryEnd(VPerfQuery *pq, struct vk_combuf_s *cb, int index) {
	if (index < 0 || index >= pq->queries.max)
		return;

	ASSERT(pq->queries.states[index] == QueryState_Began);

	vkCmdPipelineBarrier(cb->cmdbuf, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
		0, 0, NULL, 0, NULL, 0, NULL);
	vkCmdEndQuery(cb->cmdbuf, pq->pool, index);
	pq->queries.states[index] = QueryState_Ended;
}
...
VkPerformanceCounterResultKHR* vPerfQueryRead(VPerfQuery *pq, struct vk_combuf_s *cb, int index) {
	...
	ASSERT(pq->queries.states[index] == QueryState_Ended);
	pq->queries.states[index] = QueryState_Available;

	const uint32_t firstQuery = index;
	const uint32_t queryCount = 1;
	const size_t dataSize = pq->counters * sizeof(VkPerformanceCounterResultKHR);
	const VkDeviceSize stride = sizeof(VkPerformanceCounterResultKHR);
