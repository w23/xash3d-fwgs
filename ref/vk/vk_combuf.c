...
static void printAccessMask(const char *prefix, VkAccessFlags2 access) {
	...
	PRINT_FLAG(access, VK_ACCESS_2_COMMAND_PREPROCESS_READ_BIT_NV);
	PRINT_FLAG(access, VK_ACCESS_2_COMMAND_PREPROCESS_WRITE_BIT_NV);
...
static void printStageMask(const char *prefix, VkPipelineStageFlags2 stages) {
	...
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_TRANSFER_BIT);
	...
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_RAY_TRACING_SHADER_BIT_NV);
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_NV);
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_FRAGMENT_DENSITY_PROCESS_BIT_EXT);
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_TASK_SHADER_BIT_NV);
	PRINT_FLAG(stages, VK_PIPELINE_STAGE_2_MESH_SHADER_BIT_NV);
...
void R_VkCombufIssueBarrier(vk_combuf_t* combuf, r_vkcombuf_barrier_t bar) {
	...
	for (int i = 0; i < bar.buffers.count; ++i) {
		...
		if (LOG_VERBOSE) {
			...
			DEBUG("  srcAccessMask = %x", bmb.srcAccessMask);
			printAccessMask("   ", bmb.srcAccessMask);
			DEBUG("  srcStageMask = %x", bmb.srcStageMask);
			printStageMask("   ", bmb.srcStageMask);
			DEBUG("  dstAccessMask = %x", bmb.dstAccessMask);
			printAccessMask("   ", bmb.dstAccessMask);
			DEBUG("  dstStageMask = %x", bmb.dstStageMask);
