...
#define PARTICLE_PACKED_COLOR( p ) ((p)->unused)
...
void CL_DrawParticles( double frametime, particle_t *cl_active_particles, float partsize )
{
	...
	{
		if(( p->type != pt_blob ) || ( PARTICLE_PACKED_COLOR( p ) == 255 ))
		{
	...
	*/
	...
	for( p = cl_active_tracers; p; p = p->next )
	{
		...
		if( !CL_CullTracer( p, start, end ))
		{
			...
			TriColor4ub_( pColor->r, pColor->g, pColor->b, PARTICLE_PACKED_COLOR( p ) );
		...
		if( p->type == pt_grav )
		{
			...
			PARTICLE_PACKED_COLOR( p ) = 255 * (p->die - gp_cl->time) * 2;
			if( PARTICLE_PACKED_COLOR( p ) > 255 ) PARTICLE_PACKED_COLOR( p ) = 255;
		}
