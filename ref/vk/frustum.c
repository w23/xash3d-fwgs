...
void GL_FrustumInitProj( gl_frustum_t *out, float flZNear, float flZFar, float flFovX, float flFovY )
{
	...
	// FIXME this is smallest change patch, can be improved
	struct {
		vec3_t cullorigin;
	...
	VectorCopy(g_camera.vieworg, RI.cullorigin);
	VectorCopy(g_camera.vforward, RI.cull_vforward);
	VectorCopy(g_camera.vright, RI.cull_vright);
	VectorCopy(g_camera.vup, RI.cull_vup);
